...
The `/vault` command already runs this and shows you the result before you
compose a reply — treat that output as authoritative and skip re-running it,
unless the command output is missing (e.g. the skill was invoked another way),
in which case run it yourself:
