---
name: obsidian-vault
description: Operate inside an Obsidian vault — create notes, update project stubs, log sessions, capture ideas, write research notes and guides, and maintain Maps of Content. Uses a SQLite FTS index to locate notes instead of searching the filesystem. Use when the user wants something recorded, found, or updated in their vault.
---
...
<quick_start>
1. Read `references/vault-config.md` — it defines the vault root and folder names.
   Never assume a path.
2. Most common task, session logging: read `workflows/log-session.md` and follow it.
3. Anything else: match the intent-routing table in `<intake>`, then read that workflow.
</quick_start>
